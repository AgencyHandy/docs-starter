...
MCP supports both reading your workspace and taking actions (create, assign, send, update) with your API key. Use `ah_custom_field_list` then `ah_custom_field_set_values` (or pass `customFields` on lead/client create). Client chat replies are not exposed through MCP yet.
